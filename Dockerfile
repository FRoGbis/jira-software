...
RUN apt-get update \
	...
	&& chmod 700 atlassian-jira-software-7.6.0-x64.bin

ADD response.varfile /opt/atlassian/jira/.install4j/response.varfile

RUN bash +x /root/atlassian-jira-software-7.6.0-x64.bin -q -varfile /opt/atlassian/jira/.install4j/response.varfile \
	&& rm -f atlassian-jira-software-7.6.0-x64.bin
